                            ...
                            @foreach ($catgdata as $item)
                                <tr>
                                    ...
                                    <td class="px-5 py-3 whitespace-nowrap">
                                        <div x-data="{ switcherToggle: {{ $item->category_status == 1 ? 'true' : 'false' }} }">
                                            <label for="{{ $item->id }}" class="cursor-pointer">
                                                <div class="relative"><input type="checkbox" id="{{ $item->id }}"
                                                        class="sr-only changeStatus"
                                    ...
                                    <td class="px-6 py-3 whitespace-nowrap">
                                        <div class="flex items-center justify-center">
                                            <svg @click="isModalOpena = !isModalOpena" id="{{ $item->id }}"
                                                class="dlt-btn cursor-pointer hover:fill-error-500 dark:hover:fill-error-500 fill-gray-700 dark:fill-gray-400"
                ...
                <div
                    ...
                    <div class="pb-3 sm:pb-0">
                        <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">
                            Showing
                            <span class="text-gray-800 dark:text-white/90">{{$catgdata->currentPage()}}</span>
                            to
                            <span class="text-gray-800 dark:text-white/90">{{$catgdata->count()}}</span>
                            of
                            <span class="text-gray-800 dark:text-white/90">{{$catgdata->total()}}</span>
                        </span>
                    </div>
                    <div
                    ...
                    @else
                        <a href="{{ $catgdata->previousPageUrl() }}"
                    ...
                    <ul class="hidden items-center gap-0.5 sm:flex">
                        @foreach ($catgdata->getUrlRange(1, $catgdata->lastPage()) as $page => $url)
                            <li>
                                <a href="{{ $url }}"
                                    class="flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium {{ $catgdata->currentPage() == $page ? 'bg-brand-500 text-white' : 'hover:bg-brand-500 text-gray-700 dark:text-gray-400 hover:text-white dark:hover:text-white' }}">
                                    {{ $page }}
                                </a>
                            </li>
                        @endforeach
                    ...
                    <span class="block text-sm font-medium text-gray-700 sm:hidden dark:text-gray-400">
                        Page {{ $catgdata->currentPage() }} of {{ $catgdata->lastPage() }}
                    </span>

                    {{-- Next Page --}}
                    @if ($catgdata->hasMorePages())
                        <a href="{{ $catgdata->nextPageUrl() }}"
                            class="shadow-theme-xs flex items-center gap-2 rounded-lg border border-gray-300 bg-white p-2 text-gray-700 hover:bg-gray-50 hover:text-gray-800 sm:p-2.5 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                            <svg class="fill-current" width="20" height="20">
                                <path
                    ...
                    <form action="{{ route('storecategory') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                ...
                                <input type="text" placeholder="Team Manager" id="catgname" name="catgName" value="{{old('catgName')}}"
                        ...
                        <div class="flex items-center  w-full gap-3 mt-6">
                            <button type="submit" id="Dharmender"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.dlt-btn').on('click', function() {
                let id = $(this).attr('id');
                let secound = $('.dlt-confirm-btn').attr('id', id);
            });
            $('.dlt-confirm-btn').on('click', function() {
                let btn = this;
                let dltid = $(btn).attr('id');
                $.ajax({
                    type: "post",
                    url: "{{ route('dlt-ctg') }}",
                    dataType: 'json',
